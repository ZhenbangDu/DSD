...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, cur_depth=0, moe_layers=None, num_experts=6, router='cosine_top', top_k=6):
        super().__init__()
    ...
    def forward(self, x, return_attention=False, return_gates=False, return_middlegraph=False):
        if return_attention:
        ...
        if self.cur_layer == 'S':
            ...
            resi = x
            x=self.norm2(x)
            x, gates, _ = self.mlp(x, return_gates=return_gates, return_middlegraph=return_middlegraph)
            x = self.drop_path(x) +resi
            self.aux_loss = self.mlp.l_aux 
            if return_gates:
                return x, gates
            elif return_middlegraph:
                return x, _
            else:
                return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', moe_layers=None, num_experts=6, top_k=6, router='cosine_top'):
        """
        Args:
        ...
        self.backbone_feat_dim = self.num_features
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                cur_depth=i, moe_layers=moe_layers, num_experts=num_experts, router=router, top_k=top_k)
    ...
    def forward_features(self, x, domain_index=None, return_gates = False, return_middlegraph=False):
        ...
        if return_gates:
            for blk in self.blocks:
                x, gates = blk(x, domain_index, return_gates=True)
                if gates is not None:
                    allgates.append(gates)
        elif return_middlegraph:
            for blk in self.blocks:
                x, graphs = blk(x, domain_index, return_middlegraph=return_middlegraph)
        ...
        if self.dist_token is None:
            if return_gates:
                return self.pre_logits(x[:, 0]), allgates
            elif return_middlegraph:
                return self.pre_logits(x[:, 0]), allgraphs
    ...
    def save_ori_expert(self):
        
        with torch.no_grad():
            for i, block in enumerate(self.blocks.children()):
                if hasattr(block.mlp, 'experts'):
                    self.batched_fc1_w.append(getattr(block.mlp.experts,"batched_fc1_w"))
                    self.batched_fc2_w.append(getattr(block.mlp.experts,"batched_fc2_w"))
                    self.batched_fc1_bias.append(getattr(block.mlp.experts,"batched_fc1_bias"))
                    self.batched_fc2_bias.append(getattr(block.mlp.experts,"batched_fc2_bias"))
    
    def add_new_experts(self, num=0, ind=[10]):
        
        # getattr(block.mlp, f'fc{r + 1}').weight.copy_(_n2p(w[f'{block_prefix}MlpBlock_3/Dense_{r}/kernel']))
        # getattr(block.mlp, f'fc{r + 1}').bias.copy_(_n2p(w[f'{block_prefix}MlpBlock_3/Dense_{r}/bias']))
        for ith,indx in enumerate(ind):
            # self.blocks[indx].mlp.experts.batched_fc1_w
            # definition = vars(self.blocks[indx].mlp.experts)
            # print(definition)
            curr_num = getattr(self.blocks[indx].mlp, 'experts').batched_fc1_w.shape[0]
            # print(curr_num)
            drop_out = nn.Dropout(0.1)
            new_moe = tutel_moe.moe_layer(
                gate_type={'type': self.router, 'k': 2, 'fp32_gate': True, 'gate_noise': 1.0, 'capacity_factor': 1.5},
                experts={'type': 'ffn', 'count_per_node': curr_num+num,
                         'hidden_size_per_expert': 4*self.embed_dim,
                         'activation_fn': lambda x: drop_out(F.gelu(x))},
                model_dim=self.embed_dim,
                batch_prioritized_routing=True,
                is_gshard_loss=False,
            )
            # definition = vars(new_moe.experts)
            # print(definition)
            with torch.no_grad():
                for i in range(curr_num):
                    getattr(new_moe.experts, 'batched_fc1_w')[i].copy_(getattr(self.blocks[indx].mlp.experts, 'batched_fc1_w')[i])
                    getattr(new_moe.experts, 'batched_fc2_w')[i].copy_(getattr(self.blocks[indx].mlp.experts, 'batched_fc2_w')[i])
                    getattr(new_moe.experts, 'batched_fc1_bias')[i].copy_(getattr(self.blocks[indx].mlp.experts, 'batched_fc1_bias')[i])
                    getattr(new_moe.experts, 'batched_fc2_bias')[i].copy_(getattr(self.blocks[indx].mlp.experts, 'batched_fc2_bias')[i])
                for i in range(curr_num,curr_num+num):
                    getattr(new_moe.experts, 'batched_fc1_w')[i].copy_(self.batched_fc1_w[ith][0])
                    getattr(new_moe.experts, 'batched_fc2_w')[i].copy_(self.batched_fc2_w[ith][0])
                    getattr(new_moe.experts, 'batched_fc1_bias')[i].copy_(self.batched_fc1_bias[ith][0])
                    getattr(new_moe.experts, 'batched_fc2_bias')[i].copy_(self.batched_fc2_bias[ith][0])
                # definition = vars(new_moe.gates[0].gnn.layer1)
                # print(definition)
                if self.router == 'gnn_top':
                    getattr(new_moe.gates[0].gnn.layer1.lin, 'weight').copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.layer1.lin, 'weight'))
                    # getattr(new_moe.gates[0].gnn.layer1.lin, 'bias').copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.layer1.lin, 'bias'))
                    getattr(new_moe.gates[0].gnn.layer1, 'bias').copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.layer1, 'bias'))
                elif self.router == 'cosine_top':
                    getattr(new_moe.gates[0], 'cosine_projector').copy_(getattr(self.blocks[indx].mlp.gates[0], 'cosine_projector'))
                    getattr(new_moe.gates[0].gnn.layer1.lin, 'bias').copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.layer1.lin, 'bias'))
                    getattr(new_moe.gates[0].gnn.layer1, 'bias').copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.layer1, 'bias'))
                elif self.router == 'gat_top':
                    getattr(new_moe.gates[0].gnn.layer1.lin_src, 'weight').copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.layer1.lin_src, 'weight'))
                    # getattr(new_moe.gates[0].gnn.layer1.lin, 'bias').copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.layer1.lin, 'bias'))
                    getattr(new_moe.gates[0].gnn.layer1, 'bias').copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.layer1, 'bias'))
                    getattr(new_moe.gates[0].gnn.layer1, 'att_src').copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.layer1, 'att_src'))
                    getattr(new_moe.gates[0].gnn.layer1, 'att_dst').copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.layer1, 'att_dst'))
                
                # for i in range(curr_num):
                #     getattr(new_moe.gates[0].gnn.fc, 'weight')[i].copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.fc, 'weight')[i])
                #     getattr(new_moe.gates[0].gnn.fc, 'bias')[i].copy_(getattr(self.blocks[indx].mlp.gates[0].gnn.fc, 'bias')[i])
                setattr(self.blocks[indx], 'mlp', new_moe)
                self.blocks[indx].to(self.cls_token.get_device())
            # curr_num = definition['count_per_node'] 
            # print(curr_num)
